...
for trial in range(n_trials):
    ...
    match this_trial_type:
        ...
        case "right":
            middle_port_output.append(
                (Bpod.OutputChannels.PWM3, S["side_port_light_intensity"])
            )
    ...
    print(
        "Trial {0}, type  {1} info: {2}".format(
            len(bpod.session.trials), this_trial_type, bpod.session.current_trial
        ),
    )
    print("")
    ...
    print("Visited states:")
    visited_states = [
        bpod.session.current_trial.sma.state_names[i]
        for i in bpod.session.current_trial.states
    ]
